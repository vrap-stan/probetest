...
void main() {
    ...
    for(int i = 0; i < MORPHTARGETS_COUNT; i++) {
        ...
        if(morphTargetInfluences[i]
            != 0.0f) vColor += getMorph(gl_VertexID, i, 2) * morphTargetInfluences[i];
                #elif defined( USE_COLOR )
        if(morphTargetInfluences[i]
            != 0.0f) vColor += getMorph(gl_VertexID, i, 2).rgb * morphTargetInfluences[i];
            #endif
    }
    #endif
    #ifdef USE_BATCHING
    ...
    for(int i = 0; i < MORPHTARGETS_COUNT; i++) {
        if(morphTargetInfluences[i]
            != 0.0f) objectNormal += getMorph(gl_VertexID, i, 1).xyz * morphTargetInfluences[i];
    }
    #endif
    #ifdef USE_SKINNING
    ...
    for(int i = 0; i < MORPHTARGETS_COUNT; i++) {
        if(morphTargetInfluences[i]
            != 0.0f) transformed += getMorph(gl_VertexID, i, 0).xyz * morphTargetInfluences[i];
    }
    #endif
    #ifdef USE_SKINNING
    ...
    vec4 worldPosition = modelMatrix * vec4(transformed, 1.0f);
