...
#define SHADER_NAME MI_EP2-TCN304_B
#define STANDARD 
#define SHADER_NAME MI_EP2-TCN304_B
#define PROBE_COUNT 1
#define uProbe _uProbed9f64de
#define uProbeTextures _uProbeTexturesd9f64de
#define uProbeIntensity _uProbeIntensityd9f64de
#define V_ENV_MAP 1
#define USE_MAP
#define USE_LIGHTMAP
#define USE_AOMAP
...
float perspectiveDepthToViewZ( const in float depth, const in float near, const in float far ) {
    ...
    float getAlphaHashThreshold( vec3 position ) {
        float maxDeriv = max(
        length( dFdx( position.xyz ) ), length( dFdy( position.xyz ) )
        );
        float pixScale = 1.0 / ( ALPHA_HASH_SCALE * maxDeriv );
        vec2 pixScales = vec2(
        exp2( floor( log2( pixScale ) ) ), exp2( ceil( log2( pixScale ) ) )
        );
        vec2 alpha = vec2(
        hash3D( floor( pixScales.x * position.xyz ) ), hash3D( floor( pixScales.y * position.xyz ) )
        );
        float lerpFactor = fract( log2( pixScale ) );
        float x = ( 1.0 - lerpFactor ) * alpha.x + lerpFactor * alpha.y;
        float a = min( lerpFactor, 1.0 - lerpFactor );
        vec3 cases = vec3(
        x * x / ( 2.0 * a * ( 1.0 - a ) ), ( x - 0.5 * a ) / ( 1.0 - a ), 1.0 - ( ( 1.0 - x ) * ( 1.0 - x ) / ( 2.0 * a * ( 1.0 - a ) ) )
        );
...
float computeSpecularOcclusion( const in float dotNV, const in float ambientOcclusion, const in float roughness ) {
    ...
    uniform Probe _uProbed9f64de[PROBE_COUNT];
    uniform samplerCube _uProbeTexturesd9f64de[PROBE_COUNT];
    uniform float _uProbeIntensityd9f64de;
    ...
    vec4 probeColor(vec3 worldReflectVec, int i, float roughness) {
        ...
        if(i == 0) {
            envMapColor += textureCube( uProbeTextures[0], _envMapRotation * localReflectVec, roughness );
        }
        #if PROBE_COUNT > 1
            else if( i == 1) {
                envMapColor += textureCube( uProbeTextures[1], _envMapRotation * localReflectVec, roughness );
            }
        #endif
        #if PROBE_COUNT > 2
            else if( i == 2) {
                envMapColor += textureCube( uProbeTextures[2], _envMapRotation * localReflectVec, roughness );
            }
        #endif
        #if PROBE_COUNT > 3
            else if( i == 3) {
                envMapColor += textureCube( uProbeTextures[3], _envMapRotation * localReflectVec, roughness );
            }
        #endif
        #if PROBE_COUNT > 4
            else if( i == 4) {
                envMapColor += textureCube( uProbeTextures[4], _envMapRotation * localReflectVec, roughness );
            }
        #endif
        #if PROBE_COUNT > 5
            else if( i == 5) {
                envMapColor += textureCube( uProbeTextures[5], _envMapRotation * localReflectVec, roughness );
            }
        #endif
        #if PROBE_COUNT > 6
            else if( i == 6) {
                envMapColor += textureCube( uProbeTextures[6], _envMapRotation * localReflectVec, roughness );
            }
        #endif
        #if PROBE_COUNT > 7
            else if( i == 7) {
                envMapColor += textureCube( uProbeTextures[7], _envMapRotation * localReflectVec, roughness );
            }
        #endif
        #if PROBE_COUNT > 8
            else if( i == 8) {
                envMapColor += textureCube( uProbeTextures[8], _envMapRotation * localReflectVec, roughness );
            }
        #endif
        #if PROBE_COUNT > 9
            else if( i == 9) {
                envMapColor += textureCube( uProbeTextures[9], _envMapRotation * localReflectVec, roughness );
            }
        #endif
        #if PROBE_COUNT > 10
            else if( i == 10) {
                envMapColor += textureCube( uProbeTextures[10], _envMapRotation * localReflectVec, roughness );
            }
        #endif
        #if PROBE_COUNT > 11
            else if( i == 11) {
                envMapColor += textureCube( uProbeTextures[11], _envMapRotation * localReflectVec, roughness );
            }
        #endif
        #if PROBE_COUNT > 12
            else if( i == 12) {
                envMapColor += textureCube( uProbeTextures[12], _envMapRotation * localReflectVec, roughness );
            }
        #endif
        #if PROBE_COUNT > 13
            else if( i == 13) {
                envMapColor += textureCube( uProbeTextures[13], _envMapRotation * localReflectVec, roughness );
            }
        #endif
        #if PROBE_COUNT > 14
            else if( i == 14) {
                envMapColor += textureCube( uProbeTextures[14], _envMapRotation * localReflectVec, roughness );
            }
        #endif
        #if PROBE_COUNT > 15
            else if( i == 15) {
                envMapColor += textureCube( uProbeTextures[15], _envMapRotation * localReflectVec, roughness );
            }
        #endif
        #if PROBE_COUNT > 16
            else if( i == 16) {
                envMapColor += textureCube( uProbeTextures[16], _envMapRotation * localReflectVec, roughness );
            }
        #endif
...
void main() {
    ...
    float faceDirection = gl_FrontFacing ? 1.0 : - 1.0;
        ...
        #else
            mat3 tbn = getTangentFrame( - vViewPosition, normal, #if defined( USE_NORMALMAP )
                vNormalMapUv
    ...
    IncidentLight directLight;
        ...
        if(vWorldPosition.y < 0.05) {
            ...
            if(secondMinDist > 0.5) {
                ...
                float total = minDist + secondMinDist + 0.001;
